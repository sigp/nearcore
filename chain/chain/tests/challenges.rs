...
fn challenges_new_head_prev() {
    ...
    let _ = chain
        .process_block(&None, challenger_block, Provenance::PRODUCED, |_, _, _| {}, |_| {})
        .unwrap();
    ...
    if let Err(e) =
        chain.process_block(&None, last_block, Provenance::PRODUCED, |_, _, _| {}, |_| {})
    {
    ...
    let _ = chain
        .process_block(&None, b3.clone(), Provenance::PRODUCED, |_, _, _| {}, |_| {})
        .unwrap()
        .unwrap();

    let b4 = Block::empty(&b3, signer.clone());
    let new_head = chain
        .process_block(&None, b4.clone(), Provenance::PRODUCED, |_, _, _| {}, |_| {})
        .unwrap()
        .unwrap()
        .last_block_hash;
    ...
    let b3 = Block::empty(&chain.get_block(&hashes[2]).unwrap().clone(), signer.clone());
    let _ =
        chain.process_block(&None, b3.clone(), Provenance::PRODUCED, |_, _, _| {}, |_| {}).unwrap();

    let b4 = Block::empty(&b3, signer.clone());
    let _ =
        chain.process_block(&None, b4.clone(), Provenance::PRODUCED, |_, _, _| {}, |_| {}).unwrap();
